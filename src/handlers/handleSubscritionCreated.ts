...
export const handleSubscriptionCreated = async (event: Stripe.Subscription) => {
    ...
    try {
        ...
        const endDate = packageData.duration =="week"? new Date(startDate.getTime() + 7 * 24 * 60 * 60 * 1000):packageData.duration=="month"? new Date(startDate.getTime() + 30 * 24 * 60 * 60 * 1000):new Date(startDate.getTime() + 365 * 24 * 60 * 60 * 1000)
